Suppose we randomly sample companies, and then sample employees within those companies.  Let $y$ be whatever outcome we are measuring.  If there are $g=1, \cdots, G$ companies, in which there are $i=1, \cdots, m_g$ employees then we can model the outcome for individual $i$ in company $g$ as 

$$ y_{i, g} = \mu + a_g + \varepsilon_{i, g} $$

in which we assume $a_g$ are i.i.d with mean 0 and variance $\sigma^2_a$, $\varepsilon_{i, g}$ are i.id with mean 0 and variance $\sigma^2_e$ , and $a_g \perp \varepsilon_{i, g}$ (the two are independent of one another).  It can then by shown that
...
## Why Does The Sample Mean Have Larger Variance Under Clustering?
...
The grand mean, over all companies, is then
...
$$  \operatorname{Var}(\bar y) = \dfrac{1}{n^2} \sum_g\left[  m_g \sigma^2_\varepsilon + m_g^2\sigma^2_a \right] = \dfrac{\sigma^2_\varepsilon}{n} + \sigma^2_a\sum_g\dfrac{m^2_g}{n^2} $$

Hence, the clustered standard error is larger than the naive standard error.  We need to account for this, else our confidence intervals will be wrong.  Using the naive approach which ignores the clustering would instead target the estimand

$$ V_{naive} =  \dfrac{\sigma^2_\varepsilon + \sigma^2_a}{n} $$

If at least 1 cluster/company has $m_g>1$ then $V_{naive} \lt \operatorname{Var}(\bar y)$. 
